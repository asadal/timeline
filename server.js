...
app.get("/admin", async (req, res) => {
  ...
  try {
    ...
    const settingsResult = await pool.query(
      "SELECT site_title, site_description FROM settings WHERE id = 1",
    );
...
app.post("/admin/add", upload.single("image"), async (req, res) => {
  ...
  // 서울 시간(GMT+9)으로 변환
  const seoulDatetime = moment
    .tz(datetime, "Asia/Seoul")
...
app.get("/admin/edit/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query("SELECT * FROM timeline WHERE id = $1", [
      id,
    ]);
    ...
    const seoulDateTime = moment
      .tz(item.date, "Asia/Seoul")
      .format("YYYY-MM-DDTHH:mm"); // datetime-local 형식
